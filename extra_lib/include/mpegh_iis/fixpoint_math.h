...
#if   defined(__mips__)
#include "mips/fixpoint_math_mips.h"
#elif defined (__arm__)
#include "arm/fixpoint_math_arm.h"
#elif defined(__XTENSA__)
#include "xtensa/fixpoint_math_xtensa.h"
 ...
 */
CDK_INLINE INT fIsLessThan(FIXP_DBL a_m, INT a_e, FIXP_DBL b_m, INT b_e)
{
  if (a_e > b_e) {    
...
CDK_INLINE INT fIsLessThan(FIXP_SGL a_m, INT a_e, FIXP_SGL b_m, INT b_e)
{
  if (a_e > b_e) {
...
/* Version with 3 table lookup and 1 linear interpolation     */
/* Date: 22-06-2012 Arthur Tritthart, IIS Fraunhofer Erlangen */
extern const UINT exp2_tab_long[32];
extern const UINT exp2w_tab_long[32];
extern const UINT exp2x_tab_long[32];

LNK_SECTION_CODE_L1
CDK_INLINE FIXP_DBL CalcInvLdData(const FIXP_DBL x)
{
...
#define INVSQRTNORM2_LINEAR_INTERPOLATE_HQ
  ...
  CDK_ASSERT(val > FL2FXCONST_DBL(0.0));
  ...
  *shift+=2 ;      /* bias for exponent */
  ...
  FIXP_DBL regtmp= fPow2Div2(reg1);           /* a = Q^2 */
...
#error "Either define INVSQRTNORM2_NEWTON_ITERATE or INVSQRTNORM2_LINEAR_INTERPOLATE"
  ...
  if (*shift & 0x00000001) { /* odd shift values ? */
    ...
    reg2 = (FIXP_DBL) 0x5A827999; /* FL2FXCONST_DBL(0.707106781186547524400844362104849f);*/ /* 1/sqrt(2); */
#ifdef __ADSP21000__
 ...
 * \return mantissa of the result.
 */
CDK_INLINE FIXP_DBL fLog2(FIXP_DBL x_m, INT x_e, INT *result_e)
{
  FIXP_DBL result_m;
 ...
 */
CDK_INLINE FIXP_DBL fLog2(FIXP_DBL x_m, INT x_e)
{
  if ( x_m <= FL2FXCONST_DBL(0.0f) ) {
...
{
  CDK_ASSERT((intValue > 0) && (intValue < 80));
  CDK_ASSERT(intValue<80);
	return invCount[intValue];
}


#endif /* __fixpoint_math_H */
